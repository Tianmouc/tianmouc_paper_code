...
#from utils.dataloaders_tianmouc import create_dataloader
...
def run(
    ...
    for batch_i, (data_, targets, paths, shapes) in enumerate(pbar):
        
        callbacks.run('on_val_batch_start')
        t1 = time_sync()
        if cuda:
            data_ = data_.to(device, non_blocking=True)
            targets = targets.to(device)
        data_ = data_.half() if half else data_.float()  # uint8 to fp16/32
        data_ /= 255  # 0 - 255 to 0.0 - 1.0
        nb, _, height, width = data_.shape  # batch size, channels, height, width
        ...
        dt[0] += t2 - t1
        # Inference
        out, train_out = model(input_data) if training else model(input_data, augment=augment, val=True)  # inference, loss outputs
        dt[1] += time_sync() - t2
